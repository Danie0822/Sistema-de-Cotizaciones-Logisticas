...
const ClienteCreateModal = ({ show, onClose, token, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
  await clientesService.create({ nombre: nombre.trim(), contacto: contacto.trim() });
  ...
  return (
    <ModalBase
      ...
      footer={
        <>
          ...
          <button type="submit" form="cliente-create-form" className="btn btn-primary" disabled={loading}>
            {loading ? (
    ...
    >
      <form id="cliente-create-form" onSubmit={handleSubmit} autoComplete="off">
        <TextField
          id="cliente-nombre"
          label="Nombre *"
          value={nombre}
