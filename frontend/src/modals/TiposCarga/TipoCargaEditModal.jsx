...
const TipoCargaEditModal = ({ show, onClose, tipoCarga, token, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await tiposCargaService.update(tipoCarga.id, {
        nombre: nombre.trim(),
        descripcion: descripcion.trim()
      });
      showSuccessAlert('Tipo de carga actualizado');
      onSuccess && onSuccess();
      onClose();
    } catch (err) {
  ...
  return (
    <ModalBase show={show} onClose={onClose} header={<>
      <h5 className="modal-title">Editar Tipo de Carga</h5>
...
TipoCargaEditModal.propTypes = {
  show: PropTypes.bool.isRequired,
  onClose: PropTypes.func.isRequired,
  tipoCarga: PropTypes.object,
  token: PropTypes.string,
  onSuccess: PropTypes.func,
};

export default TipoCargaEditModal;
