...
const ReglaCargoEditModal = ({ show, onClose, regla, token, onSuccess, tipoCargaOptions }) => {
  ...
  useEffect(() => {
    if (show && regla) {
      setTipoCargaId(regla.tipo_carga_id || '');
      setNombreRubro(regla.nombre_rubro || '');
      setPesoMin(regla.peso_min !== undefined ? regla.peso_min : '');
      setPesoMax(regla.peso_max !== undefined ? regla.peso_max : '');
      setMetodo(regla.metodo || 'porcentaje');
      setValor(regla.valor !== undefined ? regla.valor : '');
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await reglasCargoService.update(regla.id, {
        tipo_carga_id: tipoCargaId,
        nombre_rubro: nombreRubro.trim(),
        peso_min: pesoMin === '' ? undefined : Number(pesoMin),
        peso_max: pesoMax === '' ? undefined : Number(pesoMax),
        metodo,
        valor: Number(valor),
        orden: Number(orden)
      });
      showSuccessAlert('Regla de cargo actualizada');
      onSuccess && onSuccess();
      onClose();
    } catch (err) {
  ...
  return (
    <ModalBase show={show} onClose={onClose} header={<>
      <h5 className="modal-title">Editar Regla de Cargo</h5>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <TextField label="Peso mínimo (Opcional)" type="number" value={pesoMin} onChange={e => setPesoMin(e.target.value)} error={errors.pesoMin} min={0} step="any"/>
        <TextField label="Peso máximo (Opcional)" type="number" value={pesoMax} onChange={e => setPesoMax(e.target.value)} error={errors.pesoMax} min={0} step="any"/>
        <ComboBox
          label="Método"
          value={metodo}
          onChange={setMetodo}
...
ReglaCargoEditModal.propTypes = {
  show: PropTypes.bool.isRequired,
  onClose: PropTypes.func.isRequired,
  regla: PropTypes.object,
  token: PropTypes.string,
  onSuccess: PropTypes.func,
  tipoCargaOptions: PropTypes.array.isRequired
};

export default ReglaCargoEditModal;
