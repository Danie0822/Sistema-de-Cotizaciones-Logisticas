...
const TarifaBaseCreateModal = ({ show, onClose, onSuccess, clienteOptions, tipoCargaOptions, unidadOptions }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await tarifasBaseService.create({
                cliente_id: clienteId === '' ? null : clienteId,
                tipo_carga_id: tipoCargaId,
                unidad_id: unidadId,
                precio_unitario: Number(precioUnitario),
                vigencia_desde: vigenciaDesde,
                vigencia_hasta: vigenciaHasta === '' ? null : vigenciaHasta
            });
            showSuccessAlert('Tarifa base creada');
            onSuccess && onSuccess();
            onClose();
        } catch (err) {
    ...
    return (
        <ModalBase show={show} onClose={onClose} header={
            <>
                <h5 className="modal-title">Agregar Tarifa Base</h5>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <TextField label="Precio unitario" type="number" value={precioUnitario === null ? '' : precioUnitario} onChange={e => setPrecioUnitario(e.target.value)} error={errors.precioUnitario} min={0} step="any" />
...
TarifaBaseCreateModal.propTypes = {
...
export default TarifaBaseCreateModal;
