...
module.exports = {
  async up (queryInterface, Sequelize) {
    ...
    // Crear la nueva versión - SIN parámetro p_impuestos_ids
    await queryInterface.sequelize.query(`
      ...
      BEGIN
          ...
          -- 7) Aplicar impuestos AUTOMÁTICAMENTE según tipo_carga_id
          -- Buscar todos los impuestos activos y vigentes que aplican al tipo de carga
          FOR r_impuesto IN
              SELECT imp.id, imp.tipo, imp.valor, imp.aplicable_a, imp.es_acumulativo
              FROM public.impuestos imp
              WHERE imp.deleted_at IS NULL
              AND imp.activo = true
              ...
              ORDER BY 
                  (imp.tipo_carga_id IS NOT NULL) DESC, -- Impuestos específicos primero
                  imp.nombre
          LOOP
              ...
              VALUES (
                  ...
                  r_impuesto.id,
                  v_base_impuesto,
                  CASE WHEN r_impuesto.tipo = 'porcentaje' 
  ...
  async down (queryInterface, Sequelize) {
    ...
    await queryInterface.sequelize.query(`
      DROP FUNCTION IF EXISTS public.sp_cotizar_y_guardar(uuid, uuid, uuid, numeric, uuid, text, text);
      DROP FUNCTION IF EXISTS public.sp_cotizar_y_guardar(uuid, uuid, uuid, numeric, uuid, uuid[], text, text);
      DROP FUNCTION IF EXISTS public.sp_cotizar_y_guardar(uuid, uuid, uuid, numeric, uuid, uuid[]);
      DROP FUNCTION IF EXISTS public.sp_cotizar_y_guardar(uuid, uuid, uuid, numeric, varchar, varchar, uuid);
      DROP FUNCTION IF EXISTS public.sp_cotizar_y_guardar(uuid, uuid, uuid, numeric, uuid);
